...
let infoText = "Hi <br> We have implemented a cursor visible when the mouse is pressed that follows the finger around. <br>Our implementation shows 3 text predictions, ordered by most resembled, a yellow(UP), a blue(LEFT) and a purple(RIGHT). <br>In order to select them release the mouse on top of the highlighted key. <br>You can also move you cursor by performing the same action on top of the black arrows on the right side of the keyboard";
...
{ 
  ...
  start_button.position(285 - start_button.size().width/2, display_size_pos_y_offset + 85 - start_button.size().height/2);

  info = createDiv(infoText);         // create label
  info.id('info');
  info.position(10,display_size_pos_y_offset + 40 + 3*start_button.size().height);

  image(infoImage, 150, 400, 800 * 0.4, 559 * 0.4);
}
