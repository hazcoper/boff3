...
let infoText = "Hello <br> In our aplication we have selected a layout and functionality similar to the old cellphone, the one in wich you need to press n times on a key to select the character at the n'th position <br> The first button serves as a delete and a space <br> You can move your cursor from left to rigth by swipping to the desired direction <br> <b>The cursor serves as a insert on front guide";
...
{ 
  ...
  display_size_form = createInput('16');                              // create input field
  ...
  info.id('info');
  info.position(10, height/2 + 3*start_button.size().height/2);
}
...
function drawFatFinger()
{
  ...
  //push(); 
  //fill(color(255,69,0));
  //circle(mouseX, mouseY, PPCM/6.28);
  //pop();
}
