...
let cList;
//Same thing as cList but for sugestions
let sugest;
...
//declaractio0n and initializationm for the spellchecker and text prediction
...
{  
  ignoreLetter = 1;
  if(typeOffset == currently_typed.length){
    return;
  }
  if(flag){
        typeOffset--;
        ...
        if(typeOffset > currently_typed.length-1){
          typeOffset = currently_typed.length-1;  //foi isto que eu arranjei, nao havia antes e estava a dar problemas

        }
    }
    console.log("before: ", currently_typed);
    currently_typed = listString(currently_typed, "", typeOffset);
    console.log("after: ", currently_typed);
}
  ...
  if(draw_finger_arm)
  {
    ...
    if(timepressed > limPressed){
      ...
      if(abs(mouseXInitial-mouseX) > 20){
        ...
        mouseYFinal = mouseY;       
        if (timepressed % 10 == 5){
          if(mouseYFinal - mouseYInitial < 20){   //basicamente se eu andei muito no y, entao ja nao quero que seja scroll
            console.log("This is predicts");
          }
...
function drawPrediction(typed){
  ...
  if (typeof word != 'undefined' && word != ""){
    if(currently_typed != lastSeen && !isScrolling){   //basicamente vou ver se o currently typed e diferente para nao estar sempre a fazer predict, e verifica tambem se eu estou a fazer scroll ou nao
    ...
    for(var counter = 0; counter < 3; counter++){
      if(counter < predictions.length){
        var pred = sugest[counter];
        textAlign(CENTER, CENTER);
        textFont("Arial", Math.min(PPCM*2.5/(pred.getText(predictions).length+1), PPCM/3.2));
        text(pred.getText(predictions), pred.posInit.x + (pred.posFinal.x/2), pred.posInit.y + (pred.posFinal.y/2));
      }
    }
    pop();
  }  
...
function windowResized()
{
  ...
  if(flag == 1){
    ...
    sugest = [new Prediction(0, -PPCM/2, (4/3)*PPCM, 0, 0)]
    sugest.push(new Prediction((4/3)*PPCM, -PPCM/2, (4/3)*PPCM, 0, 1));
    sugest.push(new Prediction((8/3)*PPCM, -PPCM/2, (4/3)*PPCM, 0, 2));
